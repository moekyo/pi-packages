...
| Lifecycle   | `agent-manager.ts`, `agent-runner.ts`, `agent.ts`, `parent-snapshot.ts`, `execution-state.ts`, `worktree.ts`, `worktree-state.ts`, `usage.ts` | 8       | Spawn, queue, abort, resume, turn loop, status state machine, per-agent behavior, worktree isolation |
