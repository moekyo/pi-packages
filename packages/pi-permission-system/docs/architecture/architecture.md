...
| 4   | `createMcpPermissionTargets` accumulates candidates through a `pushTarget` closure that mutates a local array and dedups via `includes` — every push site asks the array what it already holds, then acts (Tell-Don't-Ask); mutable closure state with no owner. The surrounding 71-line if-return chain (cognitive 26, CRAP 17.6) repeats the push/dedup sequence per mode                                                                                                                           | C: mutable closure state                            | `mcp-targets.ts`                                                         | 3      | 2    | 12       |
...
5. **Introduce an `McpTargetList` value object** ([#318])
   ...
   - Replace the `pushTarget` closure and local array with a small `McpTargetList` value object that owns the ordered-uniqueness invariant: `add(value)` moves the `includes` check inside the object (ignores empty/null, appends only when absent), `toArray()` returns the ordered result.
     The per-mode branches _tell_ the list to `add` instead of asking the array via `includes`.
   - Scope guard: keep the value object generic.
     The MCP-naming spelling (`${server}_${tool}`, `${server}:${tool}`, `mcp_server_${server}`) and the per-mode dispatch stay pure functions that tell the list — do not hang `addQualifiedTool`/`addServerListing` command methods on the list (that conflates ordering+uniqueness with MCP naming, a separate responsibility), and do not introduce an `McpInvocation`/`deriveTargets()` class (a one-shot transform with a constructor is a function in a class costume — no retained state across calls, no polymorphic seam).
   - Smell category: C (mutable closure state → value object that owns its invariant).
   - Outcome: the `includes`-ask is gone; the uniqueness invariant lives in one owner; the per-mode dispatch reads as a sequence of tells.
     The accumulator — the only genuine state-with-an-invariant here — is the object; the dispatch stays functional.
